...
class EventLogRecord(BaseModel):
    ...
    Note: This is a pure Pydantic model stored in KV-Storage.
    ID and audit fields are managed through EventLogRecordLite in MongoDB.
    """

    # ID field (managed by MongoDB through EventLogRecordLite)
    id: OptionalType[PydanticObjectId] = Field(
        default=None, description="Document ID, generated by MongoDB"
    )

    # Audit fields (managed by AuditBase through EventLogRecordLite)
    ...
    # Soft delete field (managed through EventLogRecordLite)
    deleted_at: OptionalType[datetime] = Field(
        default=None, description="Soft delete time"
    )
...
class EventLogRecordProjection(BaseModel):
    """
    ...
    Note: This is a pure Pydantic model derived from EventLogRecord.
    It's not stored separately - just a lighter data transfer object.
    """

    # Core fields
    id: OptionalType[PydanticObjectId] = Field(default=None, description="Record ID")
    created_at: OptionalType[datetime] = Field(default=None, description="Creation time")
    updated_at: OptionalType[datetime] = Field(
        default=None, description="Last update time"
    )
