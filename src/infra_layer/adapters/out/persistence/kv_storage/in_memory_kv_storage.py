...
Simple dict-based implementation for MemCell and EpisodicMemory key-value storage.
...
from .episodic_memory_kv_storage import EpisodicMemoryKVStorage
...
class InMemoryKVStorage(MemCellKVStorage):
    """
    In-memory implementation of MemCell KV-Storage using Python dict.
...
class InMemoryEpisodicMemoryKVStorage(EpisodicMemoryKVStorage):
    """
    In-memory implementation of EpisodicMemory KV-Storage using Python dict.
    ...
    def __init__(self):
        ...
        self._storage: Dict[str, str] = {}
        self._lock = Lock()
        logger.info("InMemoryEpisodicMemoryKVStorage initialized (dict-based)")
    ...
    async def put(self, key: str, value: str) -> bool:
        ...
        try:
            with self._lock:
                self._storage[key] = value
            logger.debug(f"InMemoryEpisodicMemoryKVStorage: PUT key={key}, size={len(value)} bytes")
            return True
        except Exception as e:
            logger.error(f"InMemoryEpisodicMemoryKVStorage: PUT failed for key={key}: {e}")
            return False
    ...
    async def get(self, key: str) -> Optional[str]:
        ...
        try:
            with self._lock:
                value = self._storage.get(key)
            if value:
                logger.debug(f"InMemoryEpisodicMemoryKVStorage: GET key={key}, found size={len(value)} bytes")
            else:
                logger.debug(f"InMemoryEpisodicMemoryKVStorage: GET key={key}, not found")
            return value
        except Exception as e:
            logger.error(f"InMemoryEpisodicMemoryKVStorage: GET failed for key={key}: {e}")
            return None
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            with self._lock:
                if key in self._storage:
                    del self._storage[key]
                    logger.debug(f"InMemoryEpisodicMemoryKVStorage: DELETE key={key}, deleted")
                else:
                    logger.debug(f"InMemoryEpisodicMemoryKVStorage: DELETE key={key}, key not found")
            return True
        except Exception as e:
            logger.error(f"InMemoryEpisodicMemoryKVStorage: DELETE failed for key={key}: {e}")
            return False
            ...
            with self._lock:
                for key in keys:
                    if key in self._storage:
                        result[key] = self._storage[key]
            logger.debug(
                f"InMemoryEpisodicMemoryKVStorage: BATCH_GET requested={len(keys)}, found={len(result)}"
            )
            return result
        except Exception as e:
            logger.error(f"InMemoryEpisodicMemoryKVStorage: BATCH_GET failed: {e}")
            return {}
            ...
            with self._lock:
                for key in keys:
                    if key in self._storage:
                        ...
                        deleted_count += 1
            logger.debug(
                f"InMemoryEpisodicMemoryKVStorage: BATCH_DELETE requested={len(keys)}, deleted={deleted_count}"
            )
            return deleted_count
        except Exception as e:
            logger.error(f"InMemoryEpisodicMemoryKVStorage: BATCH_DELETE failed: {e}")
            return 0
    ...
    async def close(self) -> None:
        ...
        try:
            with self._lock:
                size = len(self._storage)
                self._storage.clear()
            logger.info(f"InMemoryEpisodicMemoryKVStorage closed, cleared {size} entries")
        except Exception as e:
            logger.error(f"InMemoryEpisodicMemoryKVStorage: CLOSE failed: {e}")
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        try:
            ...
            logger.error(f"InMemoryEpisodicMemoryKVStorage: GET_STATS failed: {e}")
            return {"entry_count": 0, "total_size_bytes": 0}
